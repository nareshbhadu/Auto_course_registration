            ...
            const [results] = await chrome.scripting.executeScript({
                ...
                func: function (courseName, courseId) {
                    ...
                    for (let i = 0; i < elements.children.length; i++) {
                        ...
                        const registerBtn = elements?.children[i]?.children[5]?.children[0]?.children[0];
                        const isDisabled = elements?.children[i]?.children[5]?.children[0]?.children[0].disabled;
                        const bgcolor = elements?.children[i].getAttribute("bgcolor");

                        if (!currCourseName || !currCourseId) continue;

                        if (currCourseName.includes(courseName) && currCourseId.includes(courseId)) {
